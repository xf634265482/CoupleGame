...
export function resolveMinghenEffects(loadout: readonly MinghenLoadoutEntry[], context: MinghenEventContext, memory: MinghenTriggerMemory): MinghenEffectResult {
  ...
  for (const equipped of loadout) {
    ...
    switch (equipped.id) {
      ...
      case 'M22': if((context.hook==='AFTER_MOVE'||context.hook==='AFTER_ATTACK')&&context.lastAction&&context.action&&context.lastAction!==context.action){store(memory,'M22_READY');result.flags.push('CLOUD_STEP_READY');} if((context.hook==='BEFORE_MOVE'||context.hook==='BEFORE_ATTACK')&&consume(memory,'M22_READY')){result.apDelta-=1;if(equipped.level===3)result.shield+=(context.maxHp??0)*value(2);} break;
      ...
      case 'M25': if(context.hook==='BEFORE_HIT'&&context.attackerOnSandPit&&equipped.level>=3) result.damageMultiplierBonus+=value(2); break;
      case 'M26': if(context.hook==='DAMAGED'&&context.source==='TERRAIN'&&context.terrainDamage&&context.terrainDamage>0){const mult=equipped.level>=2?1-value(1):equipped.level>=1?1-value(0):1;result.heal+=Math.round(context.terrainDamage*(1-mult));if(equipped.level>=3)store(memory,'M26_READY');} if(context.hook==='BEFORE_HIT'&&consume(memory,'M26_READY')) result.damageMultiplierBonus+=value(1); break;
    }
  }
