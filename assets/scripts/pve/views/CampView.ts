...
export class CampView {
  ...
  private _floorIntel(floor: number): string {
    const text = [
      ...
      '目标：截获哨兵军令\n完成条件：在错口石墙间追上并击败携令哨兵；哨兵逃速更快，守卫会冲锋拦截。\n失败条件：哨兵逃离，或角色生命降为零 / 中途撤离。\n推荐准备：击退、减速或控场；勿只贴哨兵一步一刀，注意清挡路怪。',
      '目标：爆破碎石封锁\n完成条件：先激活火药桶，再在 7 个玩家回合内抵达爆破点引爆。\n失败条件：引爆超时、角色生命降为零或中途撤离。\n推荐准备：激活后敌人会狂暴并立刻冲锋最多 3 格；点桶后立刻赶路引爆。',
      '目标：守住五波夜袭\n完成条件：四角刷怪点整波召唤；第 3 波起若未及时清空会强制刷下一波；清空五波后出现传送门。\n失败条件：角色生命降为零或中途撤离。\n推荐准备：保留 AP 与范围攻击，优先处理后半波交叉火力。',
      '目标：击败哥布林酋长\n完成条件：利用掩体躲避重击，击败哥布林酋长。\n失败条件：角色生命降为零或中途撤离。\n推荐准备：高生命、护甲与单体爆发；重击更疼，狂暴普攻更锋利，及时清理增援。',
