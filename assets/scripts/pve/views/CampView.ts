...
export class CampView {
  ...
  private _floorIntel(floor: number): string {
    const text = [
      ...
      '目标：截获哨兵军令\n完成条件：追上并击败携令逃跑的哥布林哨兵；若哨兵抵达逃离点则失败（无阻挡约 4 怪回合）。\n失败条件：哨兵逃离，或角色生命降为零 / 中途撤离。\n推荐准备：击退、减速或高机动；勿犹豫绕路。',
      '目标：爆破碎石封锁\n完成条件：先激活火药桶，再抵达爆破点引爆。\n失败条件：角色生命降为零或中途撤离。\n推荐准备：激活后敌人会狂暴并立刻冲锋最多 3 格，预留爆发与走位；爆破点前有石块绕路税。',
      '目标：守住五波夜袭\n完成条件：四角刷怪点整波召唤，刷出后立刻压向中场；第 3–5 波冲锋更远且弓手交叉火力更强；清空五波后出现传送门。\n失败条件：角色生命降为零或中途撤离。\n推荐准备：保留 AP 与范围攻击应对后几波。',
