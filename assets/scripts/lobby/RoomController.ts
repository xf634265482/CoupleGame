...
import { leaveRoom, startRoom } from '../network/LobbyService';
...
export class RoomController extends Component {
  ...
  private _onRoomUpdate = (room: RoomVO) => {
  ...
  private _onDisbanded = () => {
    this._callbacks?.onDisbanded();
    this.stop();
  };

  private _onMatchFound = (payload: { gameId?: string }) => {
    if (payload.gameId) {
  ...
  enterRoom(roomId: string): void {
    ...
    GameWatcher.on('room_update', this._onRoomUpdate);
    GameWatcher.on('room_disbanded', this._onDisbanded);
    GameWatcher.on('match_found', this._onMatchFound);
  ...
  private async _pollRoom(): Promise<void> {
    ...
    if (!roomId || this._enteringGame) return;
  ...
  private _detachWatchers(): void {
    this._stopRoomPoll();
    GameWatcher.off('room_update', this._onRoomUpdate);
    GameWatcher.off('room_disbanded', this._onDisbanded);
    GameWatcher.off('match_found', this._onMatchFound);
