...
describe('chapter one floor 1-7 vertical slice',()=>{
 ...
 test('sentinel unobstructed route is tracked against the escape marker',()=>{const d=getChapter1FloorDefinition(4);const sentinel=d.criticalTargets[0],escape=d.criticalTargets[1];const distance=Math.abs(sentinel.x-escape.x)+Math.abs(sentinel.y-escape.y);expect(distance).toBe(12);expect(d.exitCells).toHaveLength(0);expect(d.special?.unobstructedEscapeTurns).toBe(6);});
 test('floor six is a five-wave portal survival climax',()=>{const f6=getChapter1FloorDefinition(6);expect(f6.criticalTargets).toHaveLength(4);expect(f6.special).toMatchObject({waveCount:5,prepareTurns:0,waveRushSteps:4});});
