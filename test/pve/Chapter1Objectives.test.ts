...
describe('chapter one objectives',()=>{
  ...
  test('breakthrough requires barrel activation before blast detonation',()=>{const d=createBreakthroughObjective();expect(d.apply(d.create(),{type:'BLAST_DETONATED',entityId:'F5_BLAST_TARGET'}).state.status).toBe('ACTIVE');let s=d.apply(d.create(),{type:'GUNPOWDER_ACTIVATED',entityId:'F5_BARREL'}).state;expect(s.status).toBe('ACTIVE');expect(s.data.blastTurnsLeft).toBe(7);s=d.apply(s,{type:'BLAST_DETONATED',entityId:'F5_BLAST_TARGET'}).state;expect(s.status).toBe('COMPLETE');});
  test('breakthrough fails when blast window expires',()=>{
    const d=createBreakthroughObjective();
    let s=d.apply(d.create(),{type:'GUNPOWDER_ACTIVATED',entityId:'F5_BARREL'}).state;
    for(let i=0;i<6;i+=1){
      s=d.apply(s,{type:'PLAYER_TURN_ENDED'}).state;
      expect(s.status).toBe('ACTIVE');
    }
    const failed=d.apply(s,{type:'PLAYER_TURN_ENDED'});
    expect(failed.state.status).toBe('FAILED');
    expect(failed.commands.some((c)=>c.type==='OBJECTIVE_FAILED'&&c.reason==='BLAST_TURN_LIMIT')).toBe(true);
  });
  test('wave survival force-spawns next wave after countdown on late waves',()=>{
    const d=createWaveSurvivalObjective();
    let r=d.apply(d.create(),{type:'WAVE_SPAWNED',wave:1,entityIds:['a','b']});
    r=d.apply(r.state,{type:'ENTITY_KILLED',entityId:'a'});
    r=d.apply(r.state,{type:'ENTITY_KILLED',entityId:'b'});
    r=d.apply(r.state,{type:'WAVE_SPAWNED',wave:2,entityIds:['c']});
    r=d.apply(r.state,{type:'ENTITY_KILLED',entityId:'c'});
    r=d.apply(r.state,{type:'WAVE_SPAWNED',wave:3,entityIds:['d']});
    expect(r.state.data.forceSpawnTurns).toBe(4);
    for(let i=0;i<3;i+=1){
      r=d.apply(r.state,{type:'PLAYER_TURN_ENDED'});
      expect(r.commands).toEqual([]);
    }
    r=d.apply(r.state,{type:'PLAYER_TURN_ENDED'});
    expect(r.commands).toEqual([{type:'WARN_WAVE',wave:4},{type:'SPAWN_WAVE',wave:4}]);
  });
