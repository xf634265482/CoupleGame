...
describe('PveMinghen',()=>{
 test('copies stay level one until camp synthesize; trial still gates three',()=>{let c={};for(let i=0;i<4;i+=1)c=grantCopy(c,'M01');expect(c.M01).toMatchObject({copies:4,level:1,trialCompleted:false});});
 test('hunt grants deterministic one or two progress and transitions to trial ready',()=>{let p=createDefaultProfile(1);p={...p,highestClearedFloor:2,minghenCollection:{M01:{id:'M01',level:1,copies:2,trialCompleted:false}}};p=beginTracking(p,2,'M01');let r=settleMinghen(p,challenge('HUNT'),{huntBonusAchieved:true},prev);expect(r.collection.M01).toMatchObject({copies:4,level:1});expect(r.tracking.state).toBe('TRIAL_READY');});
