...
async function handleCheckInAction(user, request = {}, nowMs = Date.now()) {
  ...
  const profileBeforeNormalize = latest.pveProfile;
  let profile = normalizeProfile(profileBeforeNormalize, nowMs);
  let gained = null;

  if (action === 'GET_STATE') {
    // rollover may rewrite checkIn; persist below if changed
  ...
  const rawCheckIn = profileBeforeNormalize && profileBeforeNormalize.checkIn;
