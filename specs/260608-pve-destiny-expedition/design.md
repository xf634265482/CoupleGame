...
- **获取分层**：楼层教学命痕走首通/追踪（主题池 ID）；通用工具（含 M39–M56）由营地「今日商会」软补货——星尘池约 4 格 + 兑换配方 3 格，广告刷新 ≤1 次/日；兑换只消耗升级里程碑之外的多余副本。同名升级保留，不做异名随机合成与残片。权威细则见 `docs/superpowers/specs/2026-07-18-minghen-acquisition-economy-design.md`；云端 `PveMinghenShop.js` + 营地命痕台 UI。
