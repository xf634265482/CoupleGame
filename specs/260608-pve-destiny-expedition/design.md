...
- 营地强化直接消耗星尘；不产生额外选择池。
- 营地支持 **三合一升品**：三件同名同品质、未锁未穿装备 → 一件高一阶品质（强化归零，`baseStat` 取平均）；按材料品质扣星尘（普通 15 / 精良 30 / 稀有 60 / 史诗 120）；传奇不可再合成。营地装备台提供显式合成区（上 1 结果、下 3 材料）；背包详情用「投入合成」，不再一键自动挑料。权威在 `PveCamp` `SYNTHESIZE`。
